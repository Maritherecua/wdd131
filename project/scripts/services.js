...
function createServiceSelectList() {
    services.forEach(service => {
        ...
        document.getElementById('service-select').appendChild(optElement);
    });
}
    createServiceSelectList();
    function creatServiceCards() {
        services.forEach(service => {
            if (service.id % 2 !== 0) {
                ...
                document.getElementById('services-container').appendChild(serviceCard);
            }
            createEventsTable()
